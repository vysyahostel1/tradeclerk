...
export function LoginForm() {
  const { setToken, setUser, navigate } = useStore();
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const res = await fetch("/api/auth/google", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: `${Date.now()}@google.com`, name: "Google User" }),
      });
      const data = await res.json();
      setToken(data.token);
      setUser(data.user);
    ...
    <div className="flex min-h-[80vh] items-center justify-center px-4 py-12">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardDescription>Sign in to your TradeClerk account</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  onClick={() => toast.info("Coming soon", "Password reset is not available in demo.")}
                >
                  Forgot password?
                </button>
              </div>
